...
public class RiYingYeSelectService {
		...
		for (Bill bill : bill_list) {
			...
			for (int j = 1; j <= 3; j++) {
				if (temp.get(j) != null) {
					...
					// BicycleType bic_type = temp.get(j).getType();//自行车类型
					// Integer bic_price = bicycle.getPrice();//对应的价格
					// Integer isDaZhe = bicycle.getIsDaZhe();//是否打折
					// Integer isJiFen = bicycle.getIsJiFen();

					// CustomerType cus_type =
					// order.getCustomer().getCustomerType();//获得用户类别

					String total1 = String.valueOf(getTatolPrice(bicycle,order.getCustomer(),Integer.valueOf((bill.getDelay().toString()))));

					String total =String.valueOf(Double.valueOf(total1.substring(0, total1.indexOf("."))) * (order.getBicyclesMap().get(temp.get(j))));
					String total_money = total.substring(0, total1.indexOf("."))+ " 元";
	...
	public static double getTatolPrice(Bicycle bicycle, Customer cus, int time) {
		...
		Boolean isDaZhe_bic = bicycle.getIsDaZhe();// 自行车是否打折
		...
		if (time > bicycle.getBaseTime()) {
			...
			consume = yushu > bicycle.getChargeTime() ? ((shang + 1) * bicycle.getPrice()) : (shang * bicycle.getPrice() + bicycle.getOverTimePrice());
	...
	public static void main(String[] args) {
		...
		new RiYingYeSelectService().getRiYinYeBill();
